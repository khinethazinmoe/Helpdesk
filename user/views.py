...
def register(request):
	if request.method == 'POST':
		...
		if user_form.is_valid() and user_profile.is_valid():
			...
			messages.success(request,"Technican has been created successfully!");
...
def edit(request):
	if request.method == "POST":
		...
		if user_form.is_valid() and profile_form.is_valid():
			user_form.save();
			profile_form.save();
			messages.success(request,"Your profile have been changed successfully!");
		else:
			messages.error(request,"Your profile cannot change, try again!");
	else:
...
def user_detail_view(request,id):
	...
	if request.method == "POST":
		...
		if user_form.is_valid() and profile_form.is_valid():
			...
			messages.error(request,"User data cannot change, try again!");
	else:
